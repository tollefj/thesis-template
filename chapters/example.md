# example chapter

this chapter demonstrates the extended syntax available in all markdown chapter files.

## Enhanced Figure Syntax

Instead of the verbose standard markdown syntax:

```markdown
![Long caption with details about experimental setup...](images/plot.jpg){#fig:oldstyle width=50% short-caption="Experimental setup"}
```

You can now use the cleaner inline syntax:

@fig[newstyle](images/part1-placeholder.jpg){w=60% short="Experimental setup"} This figure shows the experimental setup with all components labeled. The system consists of multiple interconnected modules that process data in real-time.

The syntax is much more readable: `@fig[id](path){attributes} Caption.`

## Enhanced Cross-References

Reference figures naturally with @fig[newstyle], which transpiles to the correct pandoc syntax.

## Enhanced Table Syntax

@tbl[results] Performance comparison of different methods

| Method   | Accuracy | Speed    | Memory |
|----------|----------|----------|--------|
| Baseline | 85.3%    | Fast     | Low    |
| Method A | 92.1%    | Medium   | Medium |
| Method B | 88.7%    | Slow     | High   |
| Proposed | 95.2%    | Fast     | Low    |

Reference the table with @tbl[results] for the complete results.

## Semantic Callouts

@note{This is an important concept to remember. Notes are displayed in blue boxes.}

@warning{Be careful when applying this technique. It may not work in all scenarios.}

@tip{Pro tip: Always validate your results with multiple test cases.}

## Equations and References

When you have an equation:

$$
E = mc^2
$$ {#eq:einstein}

You can reference it with @eq[einstein] in the text.

## Mixed Syntax Support

standard markdown works unchanged:

![Standard syntax figure](images/part2-placeholder.jpg){#fig:standard width=40%}

You can freely mix both syntaxes in the same document. Reference @fig[standard] and @fig[newstyle] side by side.

## extended syntax benefits

1. **More readable**: `@fig[id](path) Caption` vs `![Caption](path){#fig:id}`
2. **Less typing**: Short attribute names like `w=50%` vs `width=50%`
3. **Unified references**: Always use `@type[label]` pattern
4. **Semantic callouts**: `@note{text}` instead of `::: {.bluebox title="Note"}`
5. **Backward compatible**: All existing markdown works unchanged

## Conclusion

the build transpiles this syntax to standard markdown automatically.
